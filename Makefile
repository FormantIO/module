...
deploy: bump-minor build
	...
	wget https://github.com/FormantIO/module/archive/refs/tags/stage.zip
	rm -rf versions/stage
	mkdir -p versions/stage
	unzip -j stage.zip 'module-stage/dist/*' -d versions/stage
	rm stage.zip
	git add versions/stage
	# unzip into versions/prod
	wget https://github.com/FormantIO/module/archive/refs/tags/prod.zip
	rm -rf versions/prod
	mkdir -p versions/prod
	unzip -j prod.zip 'module-prod/dist/*' -d versions/prod
	rm prod.zip
